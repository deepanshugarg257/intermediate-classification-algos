...
def kernelize(X, Y):
    ...
    kernelX = np.append(X, np.reshape(X[:, 0]*X[:, 0] + X[:, 1]*X[:, 1], (len(X), 1)), 1)    
    # kernelX = np.append(X, np.ones((len(X), 1)), 1)    
    #print kernelX
    """
    ===========================================================================
    """
...
def perceptron(X, Y):
    ...
    # YOUR CODE GOES HERE
    p = Perceptron(fit_intercept=True, tol=None, verbose=0, eta0=1.0, n_jobs=1, random_state=0, class_weight=None, warm_start=False, n_iter=None)
    p.fit(kernelX, Y)
    train_predictions = p.predict(kernelX)
    """
    ==========================================================================
    """
    ...
    W = p.coef_[0]                # 3D normal
    intercept = p.intercept_[0]   # Distance from origin
